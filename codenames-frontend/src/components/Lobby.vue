<template>
  <div class="main-div row m-0">
    <div
      id="women-spy"
      :class="['spy-decor', { 'left-out': isMouseInMiddle }]"
    ></div>
    <div
      class="col-sm-12 col-xl-8 offset-xl-2"
      @mouseenter="isMouseInMiddle = true"
      @mouseleave="isMouseInMiddle = false"
    >
      <CreatePlayer v-if="!isPlayerSelected"> </CreatePlayer>
      ...
      <div v-else class="row pt-5 m-0 p-4">
        ...
        <div class="col-sm-12 col-lg-4">
          <div
            class="player-list white-backgrounded-div text-left row mx-0 mb-3"
          >
        ...
        <div class="col-sm-12">
          <b-input-group
            ...
            class="col-sm-12
                           ...
                           pb-lg-5"
          >
            <b-form-input id="current-url" type="text" readonly :value="path">
            </b-form-input>
            <b-input-group-append>
              <b-button squared type="button" @click="copyPath">
<!--                Copy link-->
                {{ $t("lobby.copy-link") }}
              </b-button>
            </b-input-group-append>
    ...
    <div
      id="man-spy"
      :class="['spy-decor', { 'right-out': isMouseInMiddle }]"
    ></div>
  </div>
</template>
...
export default class Lobby extends Vue {
  ...
  private navigateToGame() {
    if (this.gameId != -1 && this.gameId != null) {
      router.push({
        name: "Game",
        params: { lobbyId: this.lobbyId, gameId: "" + this.gameId }
      });
  ...
  async mounted() {
    ...
    const joined: boolean = await this.$store.dispatch("joinLobby", {
...
button {
  ...
  outline: rgba(135, 25, 75, 0.5) solid 1px;
  outline-offset: 0px;
  text-shadow: none;
